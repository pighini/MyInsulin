...
namespace mdwBunifu
{
    public partial class boxTableau : UserControl
    {
        ...
        private void GenerateTable()
        {
            ...
            foreach (string type in Model.getAllType())
            {
            ...
            foreach (Measure mes in this.Model.GetMesure(8, "DAY"))
            {
                if (mes.DateMesure != oldDate)
                {
                    addDate(mes.DateMesure.ToShortDateString());
                    addRow();
                    cpt++;
                }
        ...
        private void fillWithValue(Measure mes)
        {
            ...
            Label lbl = labels.Where(l => l.Text == mes.DateMesure.ToShortDateString()).ToArray()[0];
